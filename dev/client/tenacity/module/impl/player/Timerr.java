...
public final class Timerr extends Module {
    ...
    public static cr.f getmField() {
    	try {
    		Field hField = Config.getMinecraft().cl.getClass().getDeclaredField("m");
    		hField.setAccessible(true);
    		return (cr.f)hField.get(Config.getMinecraft().cl);
    	}catch(Exception e) {
    		e.printStackTrace();
    		return null;
    	}
    ...
    public static void setTimerSpeed(float value){
        try {
            Field hField = Config.getMinecraft().cl.getClass().getDeclaredField("g");
            hField.setAccessible(true);
            hField.set(Config.getMinecraft().cl, new cr.e(value,getgField().a()));
    ...
    public static float getTimerSpeed(){
        try {
            ...
            hField.setAccessible(true);
           return (float)hField.get(Config.getMinecraft().cl);
        } catch (NoSuchFieldException | IllegalAccessException e) {
        	return -1;
        }
    }
